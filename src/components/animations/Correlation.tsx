    ...
    <AnimContainer>
      <div className="flex gap-3 flex-1 min-h-0">
        <div className="w-[34%] shrink-0 flex flex-col">
          <div className="text-[10px] font-mono text-text-dim mb-2">Service Chain</div>
          <div className="space-y-1 flex-1">
            {services.map((s, i) => {
              ...
              return (
                <div key={i}>
                  <div className={`flex items-center justify-between px-3 py-2 rounded-md border transition-all duration-400 ${
                    active ? "border-accent/50 bg-accent/8" :
                    done ? "border-green-500/20 bg-green-500/3" :
                    "border-border/40 bg-bg-card/20"
                  }`}>
                    <div>
                      <div className={`text-[11px] font-mono ${active ? "text-accent" : done ? "text-green-500" : "text-text-muted/50"}`}>{s.name}</div>
                      <div className="text-[9px] font-mono text-text-dim">{s.sub}</div>
                    </div>
                    <div className="flex items-center gap-1.5">
                      {active && <PulseDot active />}
                      {done && <span className="text-green-500 text-[10px]">✓</span>}
                      {active && (
                        <span className="text-[8px] font-mono text-accent bg-accent/10 px-1.5 py-0.5 rounded">a7f3</span>
                      )}
                      {pending && <span className="text-[9px] text-text-dim/30">—</span>}
                    </div>
                  </div>
                  {i < services.length - 1 && (
                    <div className="flex justify-center">
                      <svg width="12" height="14" viewBox="0 0 10 12" className={`transition-colors duration-300 ${done ? "text-green-500/40" : active ? "text-accent/40" : "text-border/30"}`}>
          ...
          {f >= 1 && (
            <div className="mt-2 px-2.5 py-1.5 rounded bg-accent/5 border border-accent/10 text-center">
              <div className="text-[9px] font-mono text-text-dim">X-FlowLens-Trace-Id</div>
              <div className="text-[10px] font-mono text-accent">a7f3b2c1</div>
            </div>
          )}
        </div>
        ...
        <MiniWindow title="Trace #a7f3 — unified" className="flex-1">
          <div className="h-full overflow-auto">
            ...
            {visibleEvents.map((e, i) => {
              ...
              return (
                <div key={i} className={`px-2.5 py-1.5 border-b transition-all duration-300 ${
                  isLatest ? "bg-accent/5 border-accent/15" : "border-border/30"
                } ${i === visibleEvents.length - 1 ? "animate-[fadeIn_0.25s_ease]" : ""}`}>
                  <div className="flex items-center gap-2">
                    <div className={`w-4.5 h-4.5 rounded-full flex items-center justify-center text-[9px] font-mono shrink-0 ${
                      isLatest ? "bg-accent text-bg" : "bg-green-500/15 text-green-500"
                    }`}>
                      {isLatest ? i + 1 : "✓"}
                    </div>
                    <span className={`text-[9px] font-mono px-1.5 rounded shrink-0 ${
                      e.src === "fe" ? "bg-blue-500/10 text-blue-500" : "bg-orange-500/10 text-orange-500"
                    }`}>
                      {e.src === "fe" ? "FE" : "BE"}
                    </span>
                    <span className={`text-[10px] font-mono flex-1 min-w-0 truncate ${isLatest ? "text-text" : "text-text-muted/70"}`}>
                      {e.label}
                    </span>
                    <span className="text-[9px] font-mono text-text-dim shrink-0">{e.t}ms</span>
                  </div>
                  <div className={`overflow-hidden transition-all duration-300 ${isLatest ? "max-h-6 opacity-100 mt-0.5" : "max-h-0 opacity-0"}`}>
                    <div className="text-[9px] font-mono text-accent/70 pl-6">{e.detail}</div>
                  </div>
                </div>
              );
            })}
            {f >= 14 && (
              <div className="px-2.5 py-2.5 text-center animate-[fadeIn_0.4s_ease]">
                <div className="text-[10px] font-mono text-green-500/80">8 events · 4 services · 1 trace</div>
              </div>
            )}
          </div>
        </MiniWindow>
